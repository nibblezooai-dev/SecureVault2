...
void wipeSensitiveData(void* ptr, size_t size) {
    if (!ptr || size == 0) return;
    SecureZeroMemory(ptr, size);
    BCryptGenRandom(NULL, (PUCHAR)ptr, (ULONG)size, BCRYPT_USE_SYSTEM_PREFERRED_RNG);
}
...
bool validateVault(HANDLE hVault, VaultHeader& header, uint64_t& fileSize) {
    ...
    DWORD br;
    SetFilePointerEx(hVault, {0}, NULL, FILE_BEGIN);
...
bool deriveKey(const std::string& password, const BYTE* salt, BYTE* outEncKey, BYTE* outAuthKey) {
    ...
    if (BCryptOpenAlgorithmProvider(&hAlg, BCRYPT_SHA512_ALGORITHM, NULL, BCRYPT_ALG_HANDLE_HMAC_FLAG) != STATUS_SUCCESS) return false;
    
    std::string pwdEnc = password + "ENCRYPT";
    std::string pwdAuth = password + "AUTH";
    
    bool s1 = (BCryptDeriveKeyPBKDF2(hAlg, (PUCHAR)pwdEnc.data(), (ULONG)pwdEnc.length(), (PUCHAR)salt, 32, 500000, outEncKey, 32, 0) == STATUS_SUCCESS);
    bool s2 = (BCryptDeriveKeyPBKDF2(hAlg, (PUCHAR)pwdAuth.data(), (ULONG)pwdAuth.length(), (PUCHAR)salt, 32, 500000, outAuthKey, 32, 0) == STATUS_SUCCESS);
...
bool decryptFile(const FileEntry& entry, HANDLE hVault, const std::string& outPath, const BYTE* encKey, const BYTE* authKey) {
    ...
    SetFilePointerEx(hVault, { (LONGLONG)entry.dataOffset }, NULL, FILE_BEGIN);
...
bool verifyPassword(HANDLE hVault, const VaultHeader& header, const std::string& pwd, BYTE* outEncKey, BYTE* outAuthKey) {
    ...
    if (header.fileCount > 0) {
        SetFilePointerEx(hVault, { (LONGLONG)header.entriesOffset }, NULL, FILE_BEGIN);
...
void LockFileFlow() {
    ...
    LARGE_INTEGER pos;
    SetFilePointerEx(hTemp, {0}, &pos, FILE_CURRENT);
    ...
    SetFilePointerEx(hTemp, {0}, NULL, FILE_BEGIN);
...
void ListVerifyFlow() {
    ...
    for (uint32_t i = 0; i < header.fileCount; ++i) {
        SetFilePointerEx(hVault, { (LONGLONG)header.entriesOffset + i * (LONGLONG)sizeof(FileEntry) }, NULL, FILE_BEGIN);
        FileEntry e; DWORD br;
        if (ReadFile(hVault, &e, sizeof(e), &br, NULL) && br == sizeof(e)) {
