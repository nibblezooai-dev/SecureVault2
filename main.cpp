...
INT_PTR CALLBACK InputBoxProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
void computeHeaderHMAC(const VaultHeader& header, const BYTE* authKey, BYTE* outHMAC);
bool verifyHeaderHMAC(const VaultHeader& header, const BYTE* authKey);
...
bool deriveKey(const std::string& password, const BYTE* salt, uint32_t iterations, BYTE* outEncKey, BYTE* outAuthKey);
...
void computeVaultHMAC(HANDLE hVault, uint64_t entriesOffset, const BYTE* authKey, BYTE* outTag) {
    ...
    if ((LONG)BCryptOpenAlgorithmProvider(&hAlg, BCRYPT_SHA256_ALGORITHM, NULL, BCRYPT_ALG_HANDLE_HMAC_FLAG) != (LONG)0x00000000L) return;

    if ((LONG)BCryptCreateHash(hAlg, &hHash, NULL, 0, (PUCHAR)(void*)authKey, (ULONG)32, 0) == (LONG)0x00000000L) {
...
bool verifyVaultHMAC(HANDLE hVault, const VaultHeader& header, const BYTE* authKey) {
    ...
    return constTimeEqual(expected, header.vaultAuthTag, (size_t)32);
}
...
void computeHeaderHMAC(const VaultHeader& header, const BYTE* authKey, BYTE* outHMAC) {
    ...
    if ((LONG)BCryptCreateHash(hAlg, &hHash, NULL, 0, (PUCHAR)(void*)authKey, (ULONG)32, 0) == (LONG)0x00000000L) {
        (void)BCryptHashData(hHash, (PUCHAR)(void*)&header, (ULONG)(sizeof(VaultHeader) - 32), 0);
...
bool verifyHeaderHMAC(const VaultHeader& header, const BYTE* authKey) {
    BYTE expected[32];
    computeHeaderHMAC(header, authKey, expected);
    return constTimeEqual(expected, header.headerHMAC, (size_t)32);
}
...
bool validateVault(HANDLE hVault, VaultHeader& header, uint64_t& fileSize) {
    ...
    if (!ReadFile(hVault, &header, (DWORD)sizeof(header), &br, NULL) || br != (DWORD)sizeof(header)) return false;

    if (!constTimeEqual((const BYTE*)header.magic, (const BYTE*)"SVL2", (size_t)4)) return false;
...
bool deriveKey(const std::string& password, const BYTE* salt, uint32_t iterations, BYTE* outEncKey, BYTE* outAuthKey) {
    BCRYPT_ALG_HANDLE hAlg = NULL;
    if ((LONG)BCryptOpenAlgorithmProvider(&hAlg, BCRYPT_SHA512_ALGORITHM, NULL, BCRYPT_ALG_HANDLE_HMAC_FLAG) != (LONG)0x00000000L) return false;
    
    std::string pwdEnc = password + "ENCRYPT";
    std::string pwdAuth = password + "AUTH";
    
    bool s1 = ((LONG)BCryptDeriveKeyPBKDF2(hAlg, (PUCHAR)(void*)pwdEnc.data(), (ULONG)pwdEnc.length(), (PUCHAR)(void*)salt, (ULONG)32, (ULONGLONG)iterations, (PUCHAR)outEncKey, (ULONG)32, 0) == (LONG)0x00000000L);
    bool s2 = ((LONG)BCryptDeriveKeyPBKDF2(hAlg, (PUCHAR)(void*)pwdAuth.data(), (ULONG)pwdAuth.length(), (PUCHAR)(void*)salt, (ULONG)32, (ULONGLONG)iterations, (PUCHAR)outAuthKey, (ULONG)32, 0) == (LONG)0x00000000L);
...
void encryptName(const std::string& name, const BYTE* encKey, const BYTE* authKey, BYTE* outEncrypted, uint32_t& outNameLen, BYTE* outIV, BYTE* outAuthTag) {
    ...
    if ((LONG)BCryptOpenAlgorithmProvider(&hAlg, BCRYPT_AES_ALGORITHM, NULL, 0) != (LONG)0x00000000L) {
    ...
    (void)BCryptGenerateSymmetricKey(hAlg, &hKey, NULL, 0, (PUCHAR)(void*)encKey, (ULONG)32, 0);
    ...
    authInfo.pbAuthData = (PUCHAR)(void*)authKey;
...
bool decryptName(const BYTE* encryptedName, uint32_t nameLen, const BYTE* encKey, const BYTE* authKey, const BYTE* iv, const BYTE* authTag, std::string& outName) {
    ...
    if ((LONG)BCryptOpenAlgorithmProvider(&hAlg, BCRYPT_AES_ALGORITHM, NULL, 0) != (LONG)0x00000000L) return false;
    (void)BCryptSetProperty(hAlg, BCRYPT_CHAINING_MODE, (PUCHAR)BCRYPT_CHAIN_MODE_GCM, (ULONG)sizeof(L"ChainingModeGCM"), 0);
    (void)BCryptGenerateSymmetricKey(hAlg, &hKey, NULL, 0, (PUCHAR)(void*)encKey, (ULONG)32, 0);
    
    BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO authInfo;
    BCRYPT_INIT_AUTH_MODE_INFO(authInfo);
    authInfo.pbNonce = (PUCHAR)(void*)iv;
    authInfo.cbNonce = (ULONG)12;
    authInfo.pbTag = (PUCHAR)(void*)authTag;
    authInfo.cbTag = (ULONG)16;
    authInfo.pbAuthData = (PUCHAR)(void*)authKey;
    authInfo.cbAuthData = (ULONG)32;
    ...
    ULONG outBytes = 0;
    BYTE outBuf[256] = {0};
    LONG status = (LONG)BCryptDecrypt(hKey, (PUCHAR)(void*)encryptedName, (ULONG)nameLen, &authInfo, NULL, 0, (PUCHAR)outBuf, (ULONG)256, &outBytes, 0);
    
    (void)BCryptDestroyKey(hKey);
    (void)BCryptCloseAlgorithmProvider(hAlg, 0);
    
    if (status != (LONG)0x00000000L) {
...
// ==========================================
bool encryptFile(HANDLE hIn, HANDLE hOutVault, const BYTE* encKey, const BYTE* authKey, FileEntry& entry) {
    BYTE inBuffer[CHUNK_SIZE];
    BYTE outBuffer[CHUNK_SIZE];

    BCRYPT_ALG_HANDLE hAlg = NULL; 
    BCRYPT_KEY_HANDLE hKey = NULL;
    
    if ((LONG)BCryptOpenAlgorithmProvider(&hAlg, BCRYPT_AES_ALGORITHM, NULL, 0) != (LONG)0x00000000L) {
        SecureZeroMemory(inBuffer, sizeof(inBuffer));
        SecureZeroMemory(outBuffer, sizeof(outBuffer));
        return false;
    }
...
bool decryptFile(const FileEntry& entry, HANDLE hVault, const std::string& outPath, const BYTE* encKey, const BYTE* authKey) {
    ...
    if ((LONG)BCryptOpenAlgorithmProvider(&hAlg, BCRYPT_AES_ALGORITHM, NULL, 0) != (LONG)0x00000000L) {
        CloseHandle(hOut); (void)DeleteFileA(outPath.c_str()); 
        SecureZeroMemory(inBuffer, sizeof(inBuffer));
        SecureZeroMemory(outBuffer, sizeof(outBuffer));
        return false;
    }
...
// ==========================================
bool validatePassword(const std::string& pwd, std::string& errorMsg) {
    if (pwd.length() < (size_t)12) { errorMsg = "Must be at least 12 characters."; return false; }
...
bool verifyPassword(HANDLE hVault, const VaultHeader& header, const std::string& pwd, BYTE* outEncKey, BYTE* outAuthKey) {
    BYTE tempEnc[32];
    BYTE tempAuth[32];
    if (!deriveKey(pwd, header.salt, header.iterations, tempEnc, tempAuth)) {
    ...
    if (status == (LONG)0x00000000L && constTimeEqual(outTag, header.pwdVerifyTag, (size_t)16)) {
...
void CenterWindow(HWND hwnd, HWND hParent) {
    ...
    int x = (int)(rcParent.left + (rcParent.right - rcParent.left - (rc.right - rc.left)) / 2);
    int y = (int)(rcParent.top + (rcParent.bottom - rcParent.top - (rc.bottom - rc.top)) / 2);
...
bool guiAuthenticate(HWND hParent, HANDLE hVault, VaultHeader& header, BYTE* outEncKey, BYTE* outAuthKey) {
    ...
    if (!success) {
        ...
        computeHeaderHMAC(header, outAuthKey, header.headerHMAC);
        LARGE_INTEGER liPos; liPos.QuadPart = 0; SetFilePointerEx(hVault, liPos, NULL, FILE_BEGIN);
        DWORD bw; WriteFile(hVault, &header, sizeof(header), &bw, NULL);
        MessageBoxA(hParent, "Wrong password — vault locked", "Error", MB_ICONERROR);
        return false;
    } else {
        if (!verifyHeaderHMAC(header, outAuthKey)) {
            MessageBoxA(hParent, "Header integrity check failed", "Error", MB_ICONERROR);
            return false;
        }
...
void CreateVaultFlow() {
    ...
    deriveKey(pwd, header.salt, 500000, encKey, authKey);
    wipeSensitiveData(pwd.data(), pwd.length());
    ...
    computeHeaderHMAC(header, authKey, header.headerHMAC);
...
void UnlockFileFlow() {
    ...
    if (hGlobal) {
        ...
        *p++ = 0; // class
        ...
        CopyMemory(p, title, (SIZE_T)n * sizeof(WCHAR));
        p += n; *p++ = 0;
        ...
        *p++ = 0xFFFF; *p++ = 0x0080; 
        LPCWSTR btnOK = L"OK";
        n = lstrlenW(btnOK);
        CopyMemory(p, btnOK, (SIZE_T)n * sizeof(WCHAR));
        p += n; *p++ = 0;
        *p++ = 0;
        p = (LPWORD)ALIGN_DWORD(p);
        ...
        *p++ = 0xFFFF; *p++ = 0x0080;
        ...
        CopyMemory(p, btnCancel, (SIZE_T)n * sizeof(WCHAR));
        p += n; *p++ = 0;
        *p++ = 0;
