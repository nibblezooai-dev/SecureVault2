...
bool deriveKey(const std::string& password, const BYTE* salt, BYTE* outEncKey, BYTE* outAuthKey) {
    ...
    bool s1 = ((LONG)BCryptDeriveKeyPBKDF2(hAlg, (PUCHAR)pwdEnc.data(), (ULONG)pwdEnc.length(), (PUCHAR)salt, (ULONG)32, (ULONGLONG)500000, (PUCHAR)outEncKey, (ULONG)32, 0) == (LONG)0x00000000L);
    bool s2 = ((LONG)BCryptDeriveKeyPBKDF2(hAlg, (PUCHAR)pwdAuth.data(), (ULONG)pwdAuth.length(), (PUCHAR)salt, (ULONG)32, (ULONGLONG)500000, (PUCHAR)outAuthKey, (ULONG)32, 0) == (LONG)0x00000000L);
...
void encryptName(const std::string& name, const BYTE* encKey, const BYTE* authKey, BYTE* outEncrypted, uint32_t& outNameLen, BYTE* outIV, BYTE* outAuthTag) {
    ...
    if ((LONG)BCryptOpenAlgorithmProvider(&hAlg, BCRYPT_AES_ALGORITHM, NULL, 0) != (LONG)0x00000000L) return;
...
LRESULT CALLBACK InputBoxProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
    case WM_CREATE:
        CreateWindowExA(0, "EDIT", "", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, 30, 40, 240, 24, hwnd, (HMENU)(UINT_PTR)1, g_hInst, NULL);
        CreateWindowExA(0, "BUTTON", "OK", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, 100, 80, 100, 30, hwnd, (HMENU)(UINT_PTR)2, g_hInst, NULL);
...
void LockFileFlow() {
    ...
    if (!copySuccess) {
        MessageBoxA(g_hMainWnd, "Out of disk space or write error", "Error", MB_ICONERROR);
        CloseHandle(hTemp); (void)DeleteFileA(tempPath); wipeSensitiveData(encKey, 32); wipeSensitiveData(authKey, 32);
        CloseHandle(hVault); CloseHandle(hIn); return;
    }
    ...
    if (!MoveFileExA(tempPath, szVaultFile, MOVEFILE_REPLACE_EXISTING)) {
        MessageBoxA(g_hMainWnd, "Out of disk space or write error", "Error", MB_ICONERROR);
        (void)DeleteFileA(tempPath); wipeSensitiveData(encKey, 32); wipeSensitiveData(authKey, 32); return;
    }
